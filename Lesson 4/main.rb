...
class UserConsole
  ...
  def create_route
    ...
    routes << route
    puts 'Редактировать список станций? 1 - да, 2 - нет'
    choose = gets.chomp.to_i
    move_train(route) if choose.zero?
    menu
  end
  ...
  def unhitch_wagons
    ...
    choose = gets.chomp.to_i
    train.unhitch_wagons(choose) if wagons.nonzero?
    menu
  end
  ...
  def move_train(route)
    puts "0 - следующая станция, 1 - предыдущая станцию"
    choose = gets.chomp.to_i
    if choose == 0
